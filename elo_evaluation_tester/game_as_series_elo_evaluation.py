...
def fetch_all_series(conn):
    with conn.cursor() as cursor:
        cursor.execute("SELECT series_id, tournament_id, winning_team_odds, series_winner FROM series ORDER BY date ASC")
        return cursor.fetchall()

def fetch_all_games_by_series(conn, series_id):
    with conn.cursor() as cursor:
        cursor.execute("SELECT winning_team_odds, map_name, map_winner FROM games WHERE series_id = %s", (series_id,))
        return cursor.fetchall()
...
def main():
    ...
    try:
        ...
        accuracy, avg_brier_score = evaluate_elo_accuracy(conn, "339d9964-0bac-42b4-a88a-e75ba4e4735f")
